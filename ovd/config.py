...
def add_ovd_config(cfg):
    _C = cfg
    _C.DDPM = CN()
    _C.DDPM.NUM_DDPM_SETPS = 10
    _C.DDPM.REC_LOSS_WEIGHT = 1.0
    _C.DDPM.NORM_DDPM_SAMPLING = 'clmap'
    _C.DDPM.NUM_BOTTLENECK_LAYERS = 2
    _C.DDPM.HIDDEN_DIM = 1024
    _C.DDPM.WITH_RECONSTRUCTION = True
    _C.DDPM.WITH_REGION_TO_IMAGE = False
    _C.DDPM.LOSS_WEIGHT_REGION_TO_IMAGE = 1.0
    _C.DDPM.CTX_DIM = 0
    _C.DDPM.WITH_COND_NOISE = False
    _C.DDPM.NUM_STEPS_REGION_TO_IMAGE = 3
    _C.DDPM.NUM_STEPS_REGION_TO_TEXT = 10
    _C.DDPM.REC_LOSS_L1 = True
    _C.DDPM.WITH_MIDDLE_COND_NOISE = False
    _C.DDPM.REFINE_COND_TYPE = 'att'
    _C.DDPM.DUAL_FORWARD = False
    _C.DDPM.KL_LOSS_WEIGHT = 5.0
    _C.DDPM.WITH_OVER_SAMPLING = False
    ...
    _C.MODEL.ROI_BOX_HEAD.USE_DDPM = False  # Use weight transfer layers in zero-shot classifier head
